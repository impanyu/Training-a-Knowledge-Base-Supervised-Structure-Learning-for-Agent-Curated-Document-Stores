#!/usr/bin/env bash
# Tiny live run: one job of 6 questions, C7 single agent, ~15 LLM calls. Costs cents.
set -euo pipefail
cd "$(dirname "$0")/.."
...
python3 scripts/build_bank.py --pool data/smoke/pool.jsonl --out data/smoke/bank.json \
  --topics 1 --seed 0
python3 scripts/build_jobs.py --bank data/smoke/bank.json --jobs 2 \
  --size-lo 3 --size-hi 4 --seed 0
