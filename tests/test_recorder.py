...
off the event stream in `log()`), and the T28 per-round cumulative snapshots
-> timeseries.jsonl."""
import json
...
CLAIM_HIT = ('claimed [q0001] capital of France? (2hop, reward 100)\n'
             'deliver ONE short answer: ...\n'
             'memory: stored answer: [q0001] capital of France? -> "Paris" '
             '(F1 1.00) GOOD')
CLAIM_MISS = "claimed [q0002] longest river in France? (2hop, reward 200)"
...
def test_repeat_delivery_tallies(tmp_path):
    ...
    rec.log(_event("agent_1", "deliver_work",
                   "delivered q0002: F1 0.00 -> 0 tokens "
                   "(no better than your stored F1 1.00)"))
    rec.log(_event("agent_1", "deliver_work",
                   "delivered q0005: F1 1.00 -> 50 tokens"))
    summary = rec.write_summary(infra, rounds_used=1)
    rec.close()
...
def test_summary_carries_delivery_rows_and_the_demand_denominator(tmp_path):
    ...
    # the job layer is gone
    assert "jobs_posted" not in summary and "jobs_closed" not in summary


def test_summary_carries_contract_records(tmp_path):
    infra = demo_infra("C0")
...
def test_log_round_attributes_answers_and_memory_to_the_deliverer(tmp_path):
    ...
    assert snap1["minted"] == 300 and snap1["total_balance"] == 1300
