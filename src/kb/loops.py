...
CURATION_SKILL = (
    ...
    "with a count, so one call closes a set exactly. Use it to find what "
    "belongs under a key, and plain search when you want to see what the "
    ...
    "sentence rather than the person it wants. Write the note as a name "
    "for the key - \"People whose hobby is model trains\" - and attach "
    "the members with link_many. The same holds for relations: friendship "
    "already lives in one note per pair, so an index of a person's "
    "friends links those notes, it does not recite the names.\n"
    ...
    "in CITY\" - hundreds of instances, none of them this one. Plain "
    "search kept returning the same five residents; "
    "search_keyword([\"lives in the city of Fenmarch\"]) reports "
    "twenty-nine notes and shows them all, so the set is closed with "
    "certainty. add(\"Residents of Fenmarch\") -> n90 and "
    "link_many(n90, [...]) attaches all twenty-nine in one action. What you did not spend the budget on: "
    ...
    "be asked again.\n"
    "  A count over a relation. \"How many grandchildren does Petra "
    "Dunmore have?\" You assembled it from six scattered notes across two "
    "generations. The key here is the relation itself: "
    "search_keyword([\"is a child of\", \"Petra Dunmore\"]) gives her "
    "children exactly, then one call per child gives theirs. "
    "add(\"Grandchildren of Petra Dunmore\") and link_many it to all six "
    "makes the count a single read for anyone who asks again - and the "
    "same treatment of the other families answers every instance of the "
    "type. A relation that is asked about repeatedly deserves its own "
    "index, not just edges.\n"
    ...
    "Oakhurst?\" - answered in three steps from a single note. Answering "
    "correctly is NOT the test. The test is whether the store already "
    "carries structure that serves this type, and you check that by "
    "looking: is there a Felix Oakhurst index, and does an equivalent "
    "one exist for other people? If yes, restating s3706 would only "
    "compete with it in search and changing nothing is right. If no - if "
    "you answered correctly by searching well rather than by following "
    "structure - then the next question of this type will cost the same "
    "three steps, and there is work to do. On an unstructured store, "
    "doing nothing is almost never correct.\n"    "\nEvery note you add must earn its retrieval slot: it competes with "
    ...
    "well - which means you looked and found the structure, not merely "
    "that you answered correctly - change NOTHING. Note text is one "
