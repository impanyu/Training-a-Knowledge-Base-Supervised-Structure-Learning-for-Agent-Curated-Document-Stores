...
CURATION_SKILL = (
    ...
    "job, and neither substitutes for the other. Two complete indexes "
    "are what make such a question cheap: reading one returns the full "
    "text of every note under it, so one read yields a whole set of "
    "names, and two reads yield two sets to intersect. Link the indexes "
    "to each other as well, but for a smaller reason - so a reader that "
    "lands on one discovers the other without a further search. If the "
    "budget only stretches to one, build it "
    ...
    "n96, n101, n108, n112, n115, n118, n124, n130, n133]) - one action, "
    "all fourteen. This is what makes n90 an index rather than a label, "
    "and the completeness is what makes it safe to trust.\n"
    ...
    "  (5) index to index: the second key. add(\"People whose job is "
    "cooper\") -> n120, populate it the same way. The question is now "
    "two reads: read(n90) yields fourteen residents by name, read(n120) "
    "yields every cooper by name, and the intersection is the answer - "
    "no searching at all. Then link(n120, n90) and link(n90, n120) so a "
    "reader arriving at either index sees that the other exists; that "
    "edge saves a search, not the whole traversal. Multi-level "
    "navigation is where index-to-index edges pay most: one \"jobs\" "
    "index linking to every per-job index turns finding the right one "
    "into a read.\n"
