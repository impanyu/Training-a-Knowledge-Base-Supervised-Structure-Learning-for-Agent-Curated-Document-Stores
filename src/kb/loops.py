...
CURATION_SKILL = (
    ...
    "  - search returns five notes; search_wide returns thirty. Five is "
    "what the reader you are building for gets, and it is not enough to "
    "enumerate everything under a key - twenty-five people live in a "
    "city and the same query returns the same five however often you "
    "ask. Use search_wide when you need the whole set, plain search when "
    "you want to see what the reader would have seen.\n"
    ...
    "search kept returning the same five residents; search_wide(\"lives "
    "in the city of Fenmarch\") returns twenty-five, and a second wide "
    "search on a rephrasing adds nobody new, so the set is closed and "
    "add(\"Residents of Fenmarch\") -> n90 and link_many(n90, [...]) "
    "attaches all twenty-five in one action. What you did not spend the budget on: "
