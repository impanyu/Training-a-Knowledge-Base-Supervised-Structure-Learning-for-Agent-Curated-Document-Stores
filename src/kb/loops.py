...
TRAIN_SYSTEM = (
    ...
    "3. Make the next question of this KIND cheaper, not this one. That "
    "exact question will not be asked again; questions of its kind will, "
    "about other entities. So what you store must be ACCESS STRUCTURE, "
    "not the answer you just computed: an answer serves the one instance "
    ...
    "chain that must make that hop, whatever entity it concerns. Two "
    "facts about this environment are worth knowing. Reading a note "
    "returns that note AND the full text of every note linked to it, so "
    "a well-linked entry point can replace a sequence of uncertain "
    "searches. And everything you can build is one of two things: a note "
    "that points at other notes, or an edge between notes that already "
    "exist - including an edge between two such pointing notes, which is "
    "how navigation acquires levels. An index whose links are "
    "incomplete is worth little; one with no links is worth nothing. You "
    "will not finish a large one in a single iteration, and you are not "
    "expected to.\n"
...
def train_iteration(store, policy, q, log, epoch, n1=N1, n2=N2, k=K) -> dict:
    ...
    mem.set_task(
        ...
        + outcome
        + "You now have the gold answer and your own trajectory. Leave the "
        "store better for the next question of this kind - asked about a "
        "different entity - than it was for this one. If it already "
        "serves that kind well, change nothing. done() when finished.")
