...
TRAIN_SYSTEM = (
    ...
    "3. Build STRUCTURE, not answers. You just computed an answer; do not "
    "store it. That exact question will not be asked again, and a store "
    "full of verified answers is a cache covering one instance each. Store "
    "what made the answer FINDABLE instead. Two mechanisms do that, and "
    "they solve different halves of the reader's problem.\n"
    "   (a) LINKS - getting from one note to the next. A reader pays a "
    "step for every search and search is uncertain: it returns five notes "
    "by similarity and may miss. Reading is different - it returns that "
    "note AND the full text of every note linked to it. So an edge turns "
    "an uncertain search into a certain traversal, and the same edge "
    "serves every question that ever needs that hop. Ask which hops this "
    "question's chain had to make, then make those hops traversable for "
    "ANY entity of that kind: link a person's job, hobby, city, spouse, "
    "parents, children and friends notes to each other; link a relation "
    "note to the notes about the person it names.\n"
    "   (b) INDEXES - knowing where to start. Search is worst at the "
    "first hop, when the question names a class rather than a note: a "
    "city, a job, a hobby, a family. An index note is an entry point for "
    "one such key: named the way a searcher would phrase it, so search "
    "lands on it, and LINKED to every note belonging to that key, so one "
    "read exposes them all. Its value is the completeness of its links, "
    "not its prose - an index listing three of thirty members is worse "
    "than none, because it looks like the answer and is not. An index is "
    "not a roster of answers: do not write out who satisfies some "
    "condition, link to the notes and let the reader read them.\n"
    "Structure built either way is shape-agnostic - the same edges and "
    "entry points serve lookups, counts, joins, intersections and reverse "
    "lookups alike.\n"
...
def train_iteration(store, policy, q, log, epoch, n1=N1, n2=N2, k=K) -> dict:
    ...
    mem.set_task(
        ...
        "Before editing: name the HOPS this question's chain had to make "
        "(which note led to which), say which of them cost you a search "
        "that a link would have saved, and name the KEY you had to search "
        "for first (a city, a job, a hobby, a family) that an index note "
        "could have caught. Then build those - links for the hops, an "
        "index for the key, for any entity of that kind and not just this "
        "one. Do not store the answer you just computed. If the graph "
        "already provides them, change nothing. done() when finished.")
