...
TRAIN_SYSTEM = (
    ...
    "full of verified answers is a cache that covers one instance each. "
    "Store the PATH instead. A reader pays a step for every search, and "
    "search is uncertain - it returns five notes by similarity and may "
    "miss. Reading a note is different: it returns that note AND the full "
    "text of every note linked to it. So a linked graph converts an "
    "uncertain search into a certain traversal, and the same edge serves "
    "every question that ever needs that hop. Ask which hops this "
    "question's chain had to make, then make those hops traversable for "
    "ANY entity, not just the one you were asked about: link a person's "
    "job, hobby, city, spouse, parents, children and friends notes to each "
    "other so one read exposes them all; link a relation note to the notes "
    "about the person it names, so following the relation is a read "
    "instead of a search. Structure built this way is shape-agnostic - the "
    "same edges serve lookups, counts, joins, intersections and reverse "
    "lookups alike.\n"
    "4. If you add a note, it must earn its retrieval slot. Two kinds do. "
    "A FACT note states something the store does not already say. An ENTRY "
    "POINT note is a hub whose value is its LINKS, not its prose: it "
    "exists so a search for the class lands somewhere that leads "
    "everywhere relevant. An entry point must therefore be densely linked "
    "and named after what a searcher would type - not described as what it "
    "is. A note that neither states a new fact nor carries many links is "
    "dead weight: it consumes a retrieval slot and a read step and leaves "
...
def train_iteration(store, policy, q, log, epoch, n1=N1, n2=N2, k=K) -> dict:
    ...
    mem.set_task(
        ...
        "(which note led to which), and say which of those hops cost you a "
        "search that a link would have saved. Then make those hops "
        "traversable - for any entity of that kind, not just this one. Do "
        "not store the answer you just computed. If the graph already "
        "makes these hops traversable, change nothing. done() when "
        "finished.")
