...
CURATION_SKILL = (
    ...
    "job, and neither substitutes for the other. Build both, then link "
    "them to each other - that edge is what turns the two-hop question "
    "into two reads. If the budget only stretches to one, build it "
    ...
    "cooper\") -> n120, populate it the same way, then link(n120, n90) "
    "and link(n90, n120). Now the type is served from both sides and a "
    "reader entering from either one reaches the other in a single read "
    "- the two-hop question has become two reads. Do this now if the "
    "budget allows; otherwise it is the first thing to do next time a "
    "job is named.\n"
