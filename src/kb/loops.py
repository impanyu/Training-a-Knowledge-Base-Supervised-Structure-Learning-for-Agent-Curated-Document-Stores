...
CURATION_SKILL = (
    ...
    "  TWO LAYERS COVER ALMOST EVERYTHING. A per-entity index - one note "
    "per person, linked to every note about that person - is the "
    "substrate every type reuses: it answers any attribute lookup in one "
    "read. On top of it, an index per attribute value - a city, a job, a "
    "hobby - linked to the per-person indexes of its members. Reading an "
    "attribute index then returns a list of names, and reading a person "
    "index returns that person's facts.\n"
    "  This makes the awkward types cheap without any special casing. A "
    "join over two attributes is two reads and an intersection: read the "
    "city index for the names living there, read the job index for the "
    "names holding it, and the name in both is the answer - you never "
    "descend to the people themselves. An intersection of two sets is "
    "the same two reads. A count is one read of the index whose members "
    "are being counted. A relation walk is one read per hop, following "
    "person index to person index.\n"
    "  Note what makes this work: read() expands ONE level, giving you "
    "the text of what you read plus the text of everything it links, but "
    "not what those link to in turn. So each layer you descend costs a "
    "read, and two layers - names on top, facts below - is what most "
    "types need.\n"
    "  A third, narrower layer is worth it when a COMBINATION keeps "
    "recurring. Instead of intersecting a city index with a job index, "
    "the city index can point at per-job indexes within that city - "
    "\"coopers in Fenmarch\" - each pointing at the person indexes. The "
    "same two reads then answer by navigation rather than intersection, "
    "which is steadier: aligning a list of fourteen names against a list "
    "of ten is where a reader makes mistakes. The cost is more index "
    "notes, and they only pay if that pairing is asked about again, so "
    "build the general two layers first and add a composite one when a "
    "combination has come up more than once. Do not build composites for "
    "pairings you have seen only once - that is caching an answer with "
