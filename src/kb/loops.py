...
CURATION_SKILL = (
    ...
    "index that person's notes together. A join over job and city enters "
    "through one of them, so index residents by city or holders by job. A "
    "relation walk enters through the relation, so link relation notes to "
    "the notes about the people they name. A count enters through the "
    "owner, so link the counted items to it. Build the index the type "
    "needs, not the one that is easiest to name.\n"
    ...
    "worth little; one with no links is worth nothing. Two ways an index "
    "fails. Its key can be too broad: \"People in the knowledge base\" or "
    ...
    "carrying ten. Finish the one you started - search for its members "
    "until you have linked what you can find - before opening another. "
    "If the budget runs out mid-index, that is fine; the next question "
    "on that key continues it. Use the gold "
    ...
    "  Step two, find the KEY. A reader of this type has two ways in, the "
    "job or the city, and search handles neither: both name a class, not "
    "a note. Either key would pay; the city is the smaller set, so index "
    "residents by city.\n"
    "  Step three, locate. search(\"lives in Fenmarch\") returns n44 "
    "\"Tomas Ashford lives in Fenmarch.\" and four more residents; a "
    "second search returns four others. That is all the looking back you "
    "need - the rest is building.\n"
    ...
    "  (4) index to statement: link_many(n90, [n44, n51, n77, n82, n95, "
    "n96, n101, n108, n112]) - one action, nine members. This is what "
    "makes n90 an index rather than a label. Nine of fourteen is worth "
    "having; come back and finish it when Fenmarch next comes up, rather "
    "than starting a second index somewhere else.\n"
    ...
    "  (5) index to index: a later question about coopers makes you "
    "add(\"People whose job is cooper\") -> n120 and populate it the same "
    "way; link(n120, n90) then records that some coopers live in "
    "Fenmarch, so this very question type becomes two reads.\n"
