...
CURATION_SKILL = (
    ...
    "implies.\n"
    "  DEPTH COSTS A READ PER LEVEL. read() expands ONE level: it gives "
    "you the text of the notes linked to what you read, not the text of "
    "what those notes link to in turn. So a city index pointing at "
    "per-person indexes shows the reader fourteen person-index notes and "
    "nothing about those people - each one then costs its own read. "
    "Levels are still worth building: a per-person index is the substrate "
    "every type reuses, and a top index over all the jobs turns finding "
    "the right job index into a read. But when you know what a type asks "
    "for, also link the index straight to the notes carrying it, so the "
    "common question lands in one read rather than descending a level at "
    "a time. Depth for reuse, breadth for speed - a good index usually "
    "has both.\n"
