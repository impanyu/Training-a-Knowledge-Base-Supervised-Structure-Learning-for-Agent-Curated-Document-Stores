...
CURATION_SKILL = (
    ...
    "structure for a type; there are usually several, with different "
    "costs and different reach. Take \"which holder of JOB lives in "
    "CITY\". You could index residents by city and holders by job and let "
    "a reader read both and intersect the names. You could index each "
    "person, linking all their notes, and let the attribute indexes point "
    "at those instead of at raw statements. You could build a composite "
    "key, \"coopers in Fenmarch\", so the answer is reached by navigation "
    "instead of intersection. You could skip index notes altogether and "
    "link each person's job note to their city note, making the hop "
    "traversable without any new node. You could put a level above, one "
    "index of all the cities pointing at the per-city ones. Each of these "
    "wins something and costs something - reads, edges, notes, effort to "
    "keep complete - and which is best depends on what else the store "
    "already has and on what you expect to be asked. Judge it yourself, "
    "and invent shapes nobody listed here.\n"
    ...
    "is Tomas Ashford. The type is not \"which cooper lives in Fenmarch\" "
    "but \"which holder of JOB lives in CITY\" - hundreds of instances, "
    "none of them the one you were asked, and a reader enters it through "
    "either the job or the city. A few searches on \"lives in Fenmarch\" "
    "and its rephrasings turn up all fourteen residents and stop turning "
    "up new ones. add(\"Residents of Fenmarch\") -> n90, then "
    "link_many(n90, [...]) attaching them in one action. The same for the "
    "job side later, and an edge between the two indexes so a reader "
    "landing on one sees the other. What you did not spend the budget "
    "on: a note recording that Tomas Ashford is the cooper of Fenmarch. "
    "You may add it, and it ranks last - it serves the one question that "
