...
RETRIEVAL_SKILL = (
    ...
    "name, an attribute word), not with the question. If the first page "
    "does not hold what you need, ask for page 2 of the SAME query before "
    "you try a different wording: a new wording shows you its own first "
    "five, which mostly repeat what you have already seen. If an index "
    "for the set exists, one read of it beats any amount of paging.\n"
...
CURATION_SKILL = (
    ...
    "one action. To collect a set you PAGE THROUGH ONE QUERY: page 1, then "
    "page 2, then page 3, and so on with the SAME query until a page comes "
    "back empty or stops adding members. Rephrasing instead of paging is "
    "the mistake to avoid - a new wording shows you its own first five, "
    "which mostly repeat what you have, so ten rephrasings see less than "
    "four pages of one query. Closing one set costs six to eight pages; "
    "that is the price, and it is exactly what a future reader will not "
    "pay once the index exists.\n"
    ...
    "\nONE KEY AT A TIME. A question may name three or four keys, and your "
    "budget will not close all of them. Finish ONE: page its query to the "
    "end, attach every member with link_many, and only then start the "
    "next. One index of twenty-nine members is worth more than four of "
    "three, and the keys you did not reach are picked up by the next "
    "question that names them. Do not spread.\n"
